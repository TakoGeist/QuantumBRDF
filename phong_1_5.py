...
consts = {'ks': 0.5}
scalars = {'n': {'range': (1, 5), 'type': np.uint32}}
angles = {'phi': {'range': (0, np.pi / 2)}}


model = Model(name, log=False)
...
def embedding(data, n_qubits, layers, params):
	for i in range(n_qubits * layers):
		_i = i % n_qubits
		if _i < len(model.scalars):
			value = data[_i]
			name = model.header[_i + len(model.consts)]
			...
			qml.RY(params[i] * data[_i], wires=i)
			# qml.RY(data[i], wires=i)

	qml.Barrier()


def rot_ent(params, n_qubits, layers):
...
_rng = np.random.default_rng(2169875416541968416541354978487541)
params = _rng.random((high * 4 * n_qubits,))

for layers in range(low, high + 1):
	start = time.time()

	_start = time.gmtime(start)
	logger.info(f'[{_start.tm_hour}:{_start.tm_min}:{_start.tm_sec}]: Started {layers} layers')

	model.generate_device(n_qubits * layers)

	def _circuit(data, params):
		embedding(data, n_qubits, layers, params[: n_qubits * layers])

		rot_ent(params[n_qubits * layers :], layers, n_qubits)
